...
<table class="list" cellspacing="0">
    <tr>
        ...
        <th>{'Allowed Triggers'|i18n( 'SysInfo')}</th>
        <th>{'Used in Workflows'|i18n( 'SysInfo')}</th>
...
{foreach $eventtypelist as $typestring => $type sequence array( 'bglight', 'bgdark') as $style}
    ...
    <tr class="{$style}">
        ...
        <td>
            {$type.description|wash}
        </td>
        <td>
        </td>
        <td>
            {foreach $type.allowed_triggers as $operation => $ops}
                {*$ops|attribute(show)*}
                {foreach $ops as $opsName => $triggers}
                    {foreach $triggers as $trigger}
                        {concat($operation, '/', $opsName, ': ', $trigger)|wash()}
        ...
        <td>
            {foreach $workflows[$typestring] as $workflow}
                <a href={concat( 'workflow/view/', $workflow.workflow.id )|ezurl()}>{$workflow.workflow.name|wash()}</a>
                (
